...
echo -e "\n${BLUE} 2 Starting containers (nginx and toxiproxy)...${NC}"
...
echo -e "\n${BLUE} 4 Configuring Toxiproxy...${NC}"

EXISTING=$(curl -s http://localhost:8474/proxies | grep -c "openmeteo" || echo "0")

if [ "$EXISTING" -gt 0 ]; then
    echo -e "${YELLOW} Proxy 'openmeteo' already exists, deleting...${NC}"
...
echo -e "\n${BLUE} 5  Configured proxies:${NC}"
./scripts/toxiproxy-helper.sh list

echo -e "\n${BLUE} 6 Testing nginx (connection to Open Meteo API)...${NC}"
